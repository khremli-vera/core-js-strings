...
function removeFirstOccurrences(str, value) {
  ...
  const lastIndex = value.length + index;
  const res = str.slice(0, index) + str.slice(lastIndex);
  return str.includes(value) ? res : str;
}
...
function isPalindrome(str) {
  const newStr = str.replaceAll(/[,!?]/g, '').replaceAll(' ', '');
...
function findLongestWord(sentence) {
  const array = sentence.split(' ').sort((a, b) => b.length - a.length);
  return array[0];
}
...
function encodeToRot13(str) {
  ...
  let index;
  for (let i = 0; i < str.length; i += 1) {
    if (!usual.includes(str[i])) {
      encodedStr += str[i];
    } else {
      index = usual.indexOf(str[i]);
      encodedStr += rot13[index];
    }
  }
  return encodedStr;
}
